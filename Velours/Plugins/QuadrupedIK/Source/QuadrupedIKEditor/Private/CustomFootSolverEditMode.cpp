	...
	for (int32 i = 0; i < RuntimeNode->SpineHitPairs.Num(); i++)
	{
		if (i < RuntimeNode->SpineFeetPair.Num() && i < RuntimeNode->SpineTransformPairs.Num())
		{
			for (int32 j = 0; j < RuntimeNode->SpineFeetPair[i].FeetArray.Num(); j++)
			{
				...
				if (RuntimeNode->bIsCalcFingerJoints)
				{
					for (int32 f = 0; f < RuntimeNode->SpineTransformPairs[i].AssociatedFingerArray[j].Num(); f++)
					{
						...
						DrawDashedLine(PDI,
							RuntimeNode->FeetFingerTransformArray[i][j][f].GetLocation(),
							FingerTransform.GetLocation(),
...
{
	if ((GraphNode->Node.SolverInputData.FeetBones.IsValidIndex(SpineIndex)))
	{
...
void FCustomFootSolverEditMode::DoTranslation(FVector& InTranslation)
{
	if (bKneeSelection)
	{
		if (RuntimeNode->FootKneeOffsetArray.IsValidIndex(SpineIndex) 
			&& (RuntimeNode->SolverInputData.FeetBones.IsValidIndex(SpineIndex))
			&& (GraphNode->Node.SolverInputData.FeetBones.IsValidIndex(SpineIndex)))
		{
