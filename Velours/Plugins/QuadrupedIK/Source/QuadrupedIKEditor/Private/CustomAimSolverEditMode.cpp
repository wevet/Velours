...
FVector FCustomAimSolverEditMode::GetWidgetLocation() const
{
	...
	if (GraphNode)
	{
		...
		{
			if (RuntimeNode->ElbowBoneTransformArray.IsValidIndex(ElbowArmIndex))
			{
				const FVector PoleOffset = RuntimeNode->AimingHandLimbs[ElbowArmIndex].ElbowPoleOffset;
				const FVector BasePosition = RuntimeNode->ElbowBoneTransformArray[ElbowArmIndex].GetLocation();

				if (RuntimeNode->bIsNsewPoleMethod)
				{
					if (NSEWIndex == 1)
					{
						return BasePosition + RuntimeNode->AimingHandLimbs[ElbowArmIndex].NorthPoleOffset;
					}

					if (NSEWIndex == 2)
					{
						return BasePosition + RuntimeNode->AimingHandLimbs[ElbowArmIndex].SouthPoleOffset;
					}

					if (NSEWIndex == 3)
					{
						return BasePosition + RuntimeNode->AimingHandLimbs[ElbowArmIndex].EastPoleOffset;
					}

					if (NSEWIndex == 4)
					{
						return BasePosition + RuntimeNode->AimingHandLimbs[ElbowArmIndex].WestPoleOffset;
					}

					return (BasePosition + PoleOffset);
				}
				else
				{
					return (BasePosition + PoleOffset);
		...
		else
		{
			...
			{
				if (RuntimeNode->DebugHandTransforms.IsValidIndex(ArmIndex))
				{
...
struct FCustomAimHeadSolverHandleHitProxy : public HHitProxy
{
	DECLARE_HIT_PROXY()
	int32 HeadIndex;
	FCustomAimHeadSolverHandleHitProxy(int32 InHeadIndex) : HHitProxy(HPP_World),
		HeadIndex(InHeadIndex)
	{
	}
	virtual EMouseCursor::Type GetMouseCursor() override { return EMouseCursor::CardinalCross; }
};
IMPLEMENT_HIT_PROXY(FCustomAimHeadSolverHandleHitProxy, HHitProxy)
...
void FCustomAimSolverEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	// Draw Head Gizmo
	for (int32 Index = 0; Index < RuntimeNode->HeadTransforms.Num(); Index++)
	{
		UMaterialInstanceDynamic* KneeMaterial = UMaterialInstanceDynamic::Create(GEngine->ArrowMaterial, GEngine->ArrowMaterial);
		KneeMaterial->SetVectorParameterValue("GizmoColor", FLinearColor(FColor::Blue));

		const FMaterialRenderProxy* KneeMaterialProxy = KneeMaterial->GetRenderProxy();
		PDI->SetHitProxy(new FCustomAimHeadSolverHandleHitProxy(Index));
		...
		DrawSphere(PDI, StartTransform.GetLocation(),
			FRotator::ZeroRotator, FVector(8.0f) * Scale, K_NumSize, K_Radius, KneeMaterialProxy, SDPG_Foreground);
	...
	{
		if (RuntimeNode->bIsUseSeparateTargets && RuntimeNode->DebugHandTransforms.IsValidIndex(Index))
		{
			...
			if (RuntimeNode->HandDefaultTransformArray.IsValidIndex(Index))
			{
				DrawDashedLine(PDI,
		...
		if (RuntimeNode->bIsReachInstead)
		{
			if (RuntimeNode->bIsNsewPoleMethod)
			{
				...
				if (RuntimeNode->ElbowBoneTransformArray.IsValidIndex(Index))
				{
					FTransform BasePoleTransform = RuntimeNode->ElbowBoneTransformArray[Index];
...
bool FCustomAimSolverEditMode::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy* HitProxy, const FViewportClick& Click)
{
	...
	if (HitProxy != nullptr && HitProxy->IsA(FCustomAimHeadSolverHandleHitProxy::StaticGetType()))
	{
		FCustomAimHeadSolverHandleHitProxy* HandleHitProxy = static_cast<FCustomAimHeadSolverHandleHitProxy*>(HitProxy);
		GraphNode->Node.bIsFocusDebugtarget = false;
		HeadIndex = HandleHitProxy->HeadIndex;
		bResult = true;
		bIsElbowPoleMode = false;
		bIsNsewPoleMethod = false;
		bIsArmManipulation = false;
...
FName FCustomAimSolverEditMode::GetSelectedBone() const
{
	...
	{
		if (RuntimeNode && RuntimeNode->HeadTransforms.IsValidIndex(HeadIndex))
		{
			const FCompactPoseBoneIndex BoneIndex = RuntimeNode->HeadTransforms[HeadIndex].BoneIndex;
			if (USkeletalMeshComponent* SkelComp = GetAnimPreviewScene().GetPreviewMeshComponent())
			{
				return SkelComp->GetBoneName(BoneIndex.GetInt());
			}
		}

		return GraphNode->Node.EndSplineBone.BoneName;
	}

	if (bIsArmManipulation && RuntimeNode->AimingHandLimbs.IsValidIndex(ArmIndex))
	{
		return RuntimeNode->AimingHandLimbs[ArmIndex].HandBone.BoneName;
	}
	return NAME_None;
}
...
void FCustomAimSolverEditMode::DoRotation(FRotator& InRotation)
{
	if (bIsArmManipulation)
	{
		if (GraphNode->Node.DebugHandTransforms.IsValidIndex(ArmIndex) && RuntimeNode->DebugHandTransforms.IsValidIndex(ArmIndex))
		{

			GraphNode->Node.DebugHandTransforms[ArmIndex].SetRotation(InRotation.Quaternion() *	GraphNode->Node.DebugHandTransforms[ArmIndex].GetRotation());
			RuntimeNode->DebugHandTransforms[ArmIndex].SetRotation(InRotation.Quaternion() * RuntimeNode->DebugHandTransforms[ArmIndex].GetRotation());
		}
	}
	else
	{
