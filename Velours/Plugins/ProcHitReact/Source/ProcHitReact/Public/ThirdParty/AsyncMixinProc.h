...
//     you inherit from AsyncMixinProc, or should it be a template argument?
//enum class EAsyncMixinProcRetentionPolicy : uint8
//{
//	Default,
...
/**
 * The FAsyncMixinProc allows easier management of async loading requests, to ensure linear request handling, to make 
 * writing code much easier.  The usage pattern is as follows,
 *
 * First - inherit from FAsyncMixinProc, even if you're a UObject, you can also inherit from FAsyncMixinProc.
 *
 ...
 * are ever out of scope of the host AsyncMixinProc derived object.
 * e.g.
 ...
 * 
 * NOTE: The FAsyncMixinProc also makes it safe to pass [this] as a captured input into your lambda, because it handles 
 * unhooking everything if either your owner class is destroyed, or you cancel everything.
 *
 * NOTE: FAsyncMixinProc doesn't add any additional memory to your class.  Several classes currently handling async loading 
 * internally allocate TSharedPtr<FStreamableHandle> members and tend to hold onto SoftObjectPaths temporary state.  The 
 * FAsyncMixinProc does all of this internally with a static TMap so that all of the async request memory is stored temporarily
 * and sparsely.
 * 
 * NOTE: For debugging and understanding what's going on, you should add -LogCmds="LogAsyncMixinProc Verbose" to the command line.
 */
class PROCHITREACT_API FAsyncMixinProc : public FNoncopyable
{
protected:
	FAsyncMixinProc();

public:
	virtual ~FAsyncMixinProc();
...
private:
	/**
	 * The FLoadingState is what actually is allocated for the FAsyncMixinProc in a big map so that the FAsyncMixinProc itself holds no
	...
	{
	public:
		FLoadingState(FAsyncMixinProc& InOwner);
...
private:
	static TMap<FAsyncMixinProc*, TSharedRef<FLoadingState>> Loading;
};
 ...
 * in the proper order, just like with combining FAsyncMixinProc with your class.
 */
class PROCHITREACT_API FAsyncScope : public FAsyncMixinProc
{
public:
	using FAsyncMixinProc::AsyncLoad;

	using FAsyncMixinProc::AsyncPreloadPrimaryAssetsAndBundles;

	using FAsyncMixinProc::AsyncCondition;

	using FAsyncMixinProc::AsyncEvent;

	using FAsyncMixinProc::CancelAsyncLoading;

	using FAsyncMixinProc::StartAsyncLoading;

	using FAsyncMixinProc::IsAsyncLoadingInProgress;
};
