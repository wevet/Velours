...
FGameplayAbilitySpecHandle UWvAbilitySystemComponentBase::ApplyGiveAbility(class UWvAbilityDataAsset* AbilityData, float DamageMotion)
{
	...
	FGameplayAbilitySpec Spec(AbilityData->AbilityClass, 1, INDEX_NONE, AbilityData);

	Spec.GetDynamicSpecSourceTags().AppendTags(AbilityData->AbilityTags);
	Spec.GetDynamicSpecSourceTags().AppendTags(AbilityData->AbilityTypeTag);
...
/// <summary>
/// Apply to WvAbilityBase OnGiveAbility
/// </summary>
