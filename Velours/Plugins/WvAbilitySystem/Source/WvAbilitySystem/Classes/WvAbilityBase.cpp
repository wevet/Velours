...
void UWvAbilityBase::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	Super::OnGiveAbility(ActorInfo, Spec);


	// SourceObject (DataAsset) Ç©ÇÁèÓïÒÇéÊìæ
	if (const UWvAbilityDataAsset* DA = Cast<UWvAbilityDataAsset>(Spec.SourceObject.Get()))
	{

		DamageMotion = DA->DamageMotion;
		//AbilityTags = DA->AbilityTags;
		//AbilityTags.AppendTags(DA->AbilityTypeTag);

		CancelAbilitiesWithTag = DA->CancelAbilitiesWithTag;
		BlockAbilitiesWithTag = DA->BlockAbilitiesWithTag;
		ActivationOwnedTags = DA->ActivationOwnedTags;
		ActivationRequiredTags = DA->ActivationRequiredTags;
		ActivationBlockedTags = DA->ActivationBlockedTags;

		SourceRequiredTags = DA->SourceRequiredTags;
		SourceBlockedTags = DA->SourceBlockedTags;
		TargetRequiredTags = DA->TargetRequiredTags;
		TargetBlockedTags = DA->TargetBlockedTags;

		// override Triggers
		AbilityTriggers = DA->AbilityTriggers;
		...
		UE_LOG(LogWvAbility, Log, TEXT("[%s] : success copy da tag to abilitytag"), *FString(__FUNCTION__));
	...
	{
		UE_LOG(LogWvAbility, Error, TEXT("[%s] : cast failed UWvAbilityDataAsset !"), *FString(__FUNCTION__));
	}
}
