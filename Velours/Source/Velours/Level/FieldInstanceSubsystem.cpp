...
DEFINE_LOG_CATEGORY(LogWvField)
...
{
	UE_LOG(LogWvField, Log, TEXT("%s"), *FString(__FUNCTION__));
	...
	UE_LOG(LogWvField, Log, TEXT("Found %d FoleyEventDataAsset IDs:"), Ids.Num());
	for (auto& Id : Ids)
	{
		UE_LOG(LogWvField, Log, TEXT("  - Type: %s  Name: %s"), *Id.PrimaryAssetType.ToString(), *Id.PrimaryAssetName.ToString());
	}
#endif
...
void UFieldInstanceSubsystem::SetHour(const int32 InHour)
{
	...
	UE_LOG(LogWvField, Log, TEXT("InHour => %d, SaveHour => %d, func => %s"), InHour, GetHour(), *FString(__FUNCTION__));
...
void UFieldInstanceSubsystem::SetSkyActor(ASkyActor* NewSkyActor)
{
	if (SkyActor != NewSkyActor)
	{
		...
		UE_LOG(LogWvField, Warning, TEXT("[%s]: Modify SkyActor. [%s]"), *FString(__FUNCTION__), *GetNameSafe(SkyActor));
	}
}
#pragma endregion
...
void UFieldInstanceSubsystem::OnFoleyEventDataAssetsLoaded()
{
	...
	{
		UE_LOG(LogWvField, Warning, TEXT("Success FoleyDA: [%s]"), *FString(__FUNCTION__));
	}
}
	...
	bool& bOutFound) const
{
	...
	if (IsValid(FoleyEventDataAsset))
	{
		...
		if (Container)
		{
			...
			if (!Container->DataArray.IsEmpty())
			{
				bOutFound = true;
