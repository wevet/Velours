...
UWvInputEventComponent::UWvInputEventComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
		...
		if (CacheAnimatingAbility)
		{
			...
			if (CacheAnimatingAbility == ASC->GetAnimatingAbility())
			{
				...
				if (!bPermanentCacheInput)
				{
					...
					GetWorld()->GetTimerManager().SetTimer(ClearCacheInput_TimerHandle, [&]()
					{
						ResetCacheInput_ClearTimer();
					}, 
					Frame, false);
				}
